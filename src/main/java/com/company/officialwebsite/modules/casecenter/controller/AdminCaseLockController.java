...
/**
 * AdminCaseLockController：案例详情独占编辑锁管理接口。
 */
@RestController
@RequestMapping("/admin/api/cases/{id}/lock")
public class AdminCaseLockController {

    private static final Logger log = LoggerFactory.getLogger(AdminCaseLockController.class);

    private final EditorLockService editorLockService;

    public AdminCaseLockController(EditorLockService editorLockService) {
    ...
    public ApiResponse<LockStatusVO> getLockStatus(
        ...
        log.info("admin query case lock status id={} user={}", id, username);
        return ApiResponse.success(editorLockService.getLockStatus(EditorResourceTypeEnum.CASE, id, username, hasForceUnlockRole(authentication)));
    }
    ...
    public ApiResponse<LockStatusVO> acquireLock(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin acquire case lock id={} user={}", id, username);
        return ApiResponse.success(editorLockService.acquireLock(EditorResourceTypeEnum.CASE, id, dto, username, username, hasForceUnlockRole(authentication)));
    }
    ...
    public ApiResponse<LockStatusVO> heartbeat(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        return ApiResponse.success(editorLockService.heartbeat(EditorResourceTypeEnum.CASE, id, lockToken, username));
    }
    ...
    public ApiResponse<Void> releaseLock(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin release case lock id={} user={}", id, username);
        editorLockService.releaseLock(EditorResourceTypeEnum.CASE, id, lockToken, username);
    ...
    public ApiResponse<Map<String, Object>> forceRelease(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin force release case lock id={} user={}", id, username);
        return ApiResponse.success(editorLockService.forceRelease(EditorResourceTypeEnum.CASE, id, dto, username, hasForceUnlockRole(authentication)));
    }
