...
/**
 * AdminIndustrySolutionLockController：行业方案详情独占编辑锁管理接口。
 */
@RestController
@RequestMapping("/admin/api/industry-solutions/{id}/lock")
public class AdminIndustrySolutionLockController {

    private static final Logger log = LoggerFactory.getLogger(AdminIndustrySolutionLockController.class);

    private final EditorLockService editorLockService;

    public AdminIndustrySolutionLockController(EditorLockService editorLockService) {
    ...
    public ApiResponse<LockStatusVO> getLockStatus(
        ...
        log.info("admin query industry solution lock status id={} user={}", id, username);
        return ApiResponse.success(editorLockService.getLockStatus(EditorResourceTypeEnum.INDUSTRY_SOLUTION, id, username, hasForceUnlockRole(authentication)));
    }
    ...
    public ApiResponse<LockStatusVO> acquireLock(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin acquire industry solution lock id={} user={}", id, username);
        return ApiResponse.success(editorLockService.acquireLock(EditorResourceTypeEnum.INDUSTRY_SOLUTION, id, dto, username, username, hasForceUnlockRole(authentication)));
    }
    ...
    public ApiResponse<LockStatusVO> heartbeat(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        return ApiResponse.success(editorLockService.heartbeat(EditorResourceTypeEnum.INDUSTRY_SOLUTION, id, lockToken, username));
    }
    ...
    public ApiResponse<Void> releaseLock(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin release industry solution lock id={} user={}", id, username);
        editorLockService.releaseLock(EditorResourceTypeEnum.INDUSTRY_SOLUTION, id, lockToken, username);
    ...
    public ApiResponse<Map<String, Object>> forceRelease(
            ...
            Authentication authentication) {
        String username = resolveUsername(authentication);
        log.info("admin force release industry solution lock id={} user={}", id, username);
        return ApiResponse.success(editorLockService.forceRelease(EditorResourceTypeEnum.INDUSTRY_SOLUTION, id, dto, username, hasForceUnlockRole(authentication)));
    }
